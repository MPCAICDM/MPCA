...
class AbstractTransformer(abc.ABC):
    ...
    def transform_one(self, x, t_ind):
        return self._transformation_list[t_ind](x)


class Rotate4D(AbstractTransformer):
    def __init__(self):
        super().__init__()

    def _create_transformation_list(self):
        transformation_list = []
        for is_filp, tx, k_rotate in itertools.product((False, True),(0, -8, 8), range(4)):
            transformation = AffineTransformation(is_filp, tx, 0, k_rotate)
