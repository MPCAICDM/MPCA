...
from outlier_datasets import load_mnist_with_outliers
...
from models.aeinae_pytorch import AeInAe_pytorch
...
def train_aeinae(trainloader, model, criterions, optimizer, epochs, trainset, y_train,lamda=1):
    model.train()
    losses = AverageMeter()
    for epoch in range(epochs):
        testloader = data.DataLoader(trainset, batch_size=1024, shuffle=False)
        a = test_aeinae_pytorch(testloader, model)
        a = a - a.min()
        a = a / (1e-8 + a.max())
        scores = 1 - a
        save_roc_pr_curve_data(scores, y_train, "1.1", mode="SHOW")

        for batch_idx, (inputs, _) in enumerate(trainloader):
            inputs = torch.autograd.Variable(inputs.cuda())
            out, rep, aep, oaep = model(inputs)
            loss = criterions[0](inputs, out) + lamda * criterions[1](rep, oaep)
...
def train_lsa(trainloader, model, criterions, optimizer, epochs, trainset, y_train):
    model.train()
    losses = AverageMeter()
    for epoch in range(epochs):
        testloader = data.DataLoader(trainset, batch_size=1024, shuffle=False)
        scores = test_lsa_pytorch(testloader, model, criterions, True)
        save_roc_pr_curve_data(scores, y_train, "1.1", mode="SHOW")

        for batch_idx, (inputs, _) in enumerate(trainloader):
            inputs = torch.autograd.Variable(inputs.cuda())
            x_r, z, z_dist = model(inputs)
            loss = criterions(inputs, x_r, z, z_dist)
            losses.update(loss.item(), inputs.size(0))
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
def test_aeinae_pytorch(testloader, model):
    model.eval()
    losses = []
    #reps = []
    for batch_idx, (inputs, _) in enumerate(testloader):
        inputs = torch.autograd.Variable(inputs.cuda())
        out, rep, aep, oaep = model(inputs)
        loss = out.sub(inputs).pow(2).view(out.size(0), -1).sum(dim=1, keepdim=False)
        losses.append(loss.data.cpu())
    losses = torch.cat(losses, dim=0)
    return losses.numpy()

def test_lsa_pytorch(testloader, model, critirions, score_norm=True):
    model.eval()
    reloss = np.zeros(shape=len(testloader.dataset,))
    aurloss = np.zeros(shape=len(testloader.dataset,))
    cc = 0
    for batch_idx, (inputs, _ ) in enumerate(testloader):
        x = inputs.cuda()
        with torch.no_grad():
            x_r, z, z_dist = model(x)
            loss = critirions(x, x_r, z, z_dist)
            bs = len(inputs)
            reloss[cc:cc+bs] = -critirions.reconstruction_loss.cpu().numpy()
            aurloss[cc:cc+bs] = -critirions.autoregression_loss.cpu().numpy()
            cc += bs
    if score_norm:
...
def _E3Outlier_experiment(x_train, y_train, dataset_name, single_class_ind, gpu_q, p):
    ...
    for epoch in range(epochs):
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            if batch_idx % 10 == 0:
                print('Epoch: [{} | {}], batch: {}, loss: {}, Accuracy: {}'.format(epoch + 1, epochs, batch_idx + 1,
                                                                                   losses.avg, top1.avg))
    ...
    x_train = [unloader(np.ceil((i + 1) / 2 * 255.)) for i in x_train]
    save_false_sample(scores, x_train, y_train, res_file_path[:-3] + "jpg")
    gpu_q.put(gpu_to_use)


def _lsa_pytorch_experiment(x_train, y_train, dataset_name, single_class_ind, gpu_q, p):
    gpu_to_use = gpu_q.get()
    ...
    if dataset_name in ['mnist', 'fashion-mnist']:
        model = LSA_MNIST(input_shape=(n_channels, h, w),code_length=code_length,cpd_channels=cpd_channels) #TODO
    else:
        return ValueError("Not implementation")
    model = model.cuda()
    cudnn.benchmark = True

    criterions = LSALoss(cpd_channels=cpd_channels,lam=lam)

    optimizer = optim.Adam(model.parameters(), eps=1e-7, weight_decay=0.0005)
    epochs = 250

    train_lsa(trainloader, model, criterions, optimizer, epochs, trainset, y_train)

    testloader = data.DataLoader(trainset, batch_size=1024, shuffle=False)
    scores = test_lsa_pytorch(testloader, model, criterions, True)

    res_file_name = '{}_lsa-{}_{}_{}.npz'.format(dataset_name, p,
                                                    get_class_name_from_index(single_class_ind, dataset_name),
                                                    datetime.now().strftime('%Y-%m-%d-%H%M'))
    res_file_path = os.path.join(RESULTS_DIR, dataset_name, res_file_name)
    os.makedirs(os.path.join(RESULTS_DIR, dataset_name), exist_ok=True)
...
def _aeinae_pytorch_experiment(x_train, y_train, dataset_name, single_class_ind, gpu_q, p):
    gpu_to_use = gpu_q.get()

    n_channels = x_train.shape[get_channels_axis()]
    model = AeInAe_pytorch(in_channels=n_channels)
    batch_size = 128
    ...
    # #########################Training########################
    train_aeinae(trainloader, model, criterions, optimizer, epochs, trainset, y_train)

    # #########################Testing########################
    testloader = data.DataLoader(trainset, batch_size=1024, shuffle=False)
    losses = test_aeinae_pytorch(testloader, model)
    losses = losses - losses.min()
    losses = losses / (1e-8 + losses.max())
    scores = 1 - losses

    res_file_name = '{}_aeinae-{}_{}_{}.npz'.format(dataset_name, p,
                                                  get_class_name_from_index(single_class_ind, dataset_name),
                                                  datetime.now().strftime('%Y-%m-%d-%H%M'))
    res_file_path = os.path.join(RESULTS_DIR, dataset_name, res_file_name)
    os.makedirs(os.path.join(RESULTS_DIR, dataset_name), exist_ok=True)
...
def run_experiments(load_dataset_fn, dataset_name, q, n_classes, abnormal_fraction, run_idx):
    ...
    for c in range(n_classes):
        ...
        #_lsa_pytorch_experiment(x_train, y_train, dataset_name, c, q, abnormal_fraction)
        #continue
        # AeInAe
        #_aeinae_pytorch_experiment(x_train, y_train, dataset_name, c, q, abnormal_fraction)
        #continue
        # CAE / CAE-IF
...
if __name__ == '__main__':
    ...
    try:
        a = nn.Linear(20, 20)
        b = torch.autograd.Variable(torch.randn(128, 20))
    ...
    experiments_list = [
        ...
        #(load_fashion_mnist_with_outliers, 'fashion-mnist', 10), TODO
