...
from loss_functions.autoregression_loss import AutoregressionLoss
...
class LSALoss(nn.Module):
    ...
    def forward(self, x, x_r, z, z_dist):
        ...
        rec_loss = self.reconstruction_loss_fn(x, x_r, False)
        arg_loss = self.autoregression_loss_fn(z, z_dist)
        tot_loss = rec_loss.mean() + self.lam * arg_loss.mean()
        print(rec_loss.mean().item() , arg_loss.mean().item(), self.lam)
        # Store numerical
