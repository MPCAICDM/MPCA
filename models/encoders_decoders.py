...
class CAE_group_pytorch(nn.Module):
    def __init__(self, in_channels = 3, rep_dim = 256, group=2):
        super(CAE_group_pytorch, self).__init__()
        nf = 64
        self.nf = nf
        self.group = group
        ...
        self.dec_conv3 = nn.ConvTranspose2d(in_channels=nf, out_channels=nf, kernel_size=3, stride=2, padding=1, output_padding=1)
        self.dec_bn3 = nn.BatchNorm2d(num_features=nf)
        self.dec_act3 = nn.ReLU(inplace=True)

        self.dec_conv4 = nn.Conv2d(in_channels=nf, out_channels=group*in_channels, kernel_size=1,groups=group, bias=False)
        self.output_act = nn.Tanh()
    ...
    def decode(self, rep):
        ...
        x_r = torch.chunk(x, self.group, dim=1)
        return x_r, reduce(add, x_r) / self.group
